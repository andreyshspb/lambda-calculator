...
make_unique :: Symb -> Set Symb -> Set Symb -> Symb
make_unique n f s = if   member n f || member n s 
                    then make_unique ('\'' : n) f s
                    else n
...
subst v n buf@(Lam s e) | s == v    = buf
                        | otherwise = if   member s (freeVars n)
                                      then let name = make_unique s (freeVars n) (freeVars e)
...
alphaEq (Lam s1 e1) (Lam s2 e2) | s1 == s2  = e1 `alphaEq` e2
                                | otherwise = if   member s1 $ freeVars e2 
                                              then False
                                              else e1 `alphaEq` (subst s2 (Var s1) e2)
